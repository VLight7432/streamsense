  ...
  try {
    ...
    if (!license.valid) {
      if (!isProd) {
        ...
        req.license = { valid: false, plan: 'free', email: null, features: {}, reason: license.reason || 'invalid' };
        return next();
      }
    ...
    // Licence valide
    req.license = license; // { valid, plan, email, features: {...} }
    return next();
  } catch (e) {
    if (!isProd) {
      // En dev/bêta : ne pas bloquer si le service de licence est KO
      req.license = { valid: false, plan: 'free', email: null, features: {}, reason: 'license_service_error' };
      return next();
    }
